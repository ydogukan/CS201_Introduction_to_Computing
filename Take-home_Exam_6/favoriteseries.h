...
{
	public:
		FavoriteSeries(string uName, string uSurname);	// constructor with parameter
		...
		bool isConflict(const series & inSeries) const;	// returns true if a given TV series has a time conflict
		...
		int findSeries(const vector<series> & favSeries, int seriesCode) const;
		vector<episode> allEpisodesSorted(const vector<series> & favSeries) const;
		int findActor(const vector<actor> & actors, string actorName) const;
		void sortActors(vector<actor> & actors) const;
